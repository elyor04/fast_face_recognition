from cv2 import (
    INTER_LINEAR, VideoCapture, waitKey, resize, INTER_AREA, circle, line, fillPoly,
    putText, FONT_HERSHEY_COMPLEX_SMALL, imshow, destroyAllWindows, drawContours)
...
def getDouble(lst:list, last=True) -> list:
    ...
    for i in range(mx+1):
        if i != mx:
            ...
            if last: r_lst.append((lst[i],lst[0]))
    return r_lst
...
while True:
    ...
    rgb_img = resize(bgr_img, (0,0), fx=0.3, fy=0.3, interpolation=INTER_AREA)[:, :, ::-1]
    ...
    prevTime = currTime
    putText(bgr_img, f"FPS: {fps}", (5,20), FONT_HERSHEY_COMPLEX_SMALL, 1.0, (250,0,0), 1)
    imshow("BGR video", bgr_img)

    if waitKey(1) & 0xFF == ord('p'): n = 0
    else: n = 3
